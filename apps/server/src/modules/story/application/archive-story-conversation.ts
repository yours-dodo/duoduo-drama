...
export class ArchiveStoryConversation {
  constructor(
  ...
  execute(input: {
    ...
    return this.transactions.run(async () => {
      ...
      const changed = conversation.archive(
      ...
      const archived = conversation.toSnapshot();
      await this.conversations.update(archived);
      return { conversation: conversationOutput(archived) };
    });
  }
}
