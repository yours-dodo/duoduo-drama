...
export class UpdateStoryProject {
  ...
  execute(input: {
    ...
    return this.transactions.run(async () => {
      ...
      if (before.visibility === 'team' && updated.visibility === 'private') {
...
function accessOutput(access: {
  ...
  subject: { collaborator: boolean; role: 'admin' | 'member'; userId: string };
}) {
  ...
  return {
    ...
    canManageCollaborators:
      ...
      (access.subject.role === 'admin' ||
        owner.createdByUserId === access.subject.userId),
  };
}
