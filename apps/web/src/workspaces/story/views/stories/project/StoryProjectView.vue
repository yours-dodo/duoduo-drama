...
const moduleDefinitions: Record<StoryModule, StoryModuleDefinition> = {
  basic: {
    ...
    workspaceDescription:
      '先把故事是什么、写给谁以及想表达什么说清楚，再开始组织世界观、角色和情节。',
    focus: [
      '故事标题与一句话概念',
      '故事的表达与目标读者',
      '故事简介和核心表达',
...
const currentTitleId = computed(() => {
  ...
  if (currentModule.value === 'basic') return undefined;
  return `story-project-${currentModule.value}-title`;
});
const currentAriaLabel = computed(() =>
  currentModule.value === 'basic'
    ? '故事基础信息'
    : currentModule.value === 'outline'
      ? '大纲工作区'
      : undefined,
);
const currentModuleKicker = computed(() => {
  if (currentModule.value === 'basic') return '项目起点 / BASIC INFO';
  if (currentModule.value === 'worldview') return '基础资产 / WORLDVIEW';
  if (currentModule.value === 'roles') return '动力资产 / ROLES';
  if (currentModule.value === 'outline') return '结构资产 / OUTLINE';
  return undefined;
});
const currentDefinition = computed(
  () => moduleDefinitions[currentModule.value],
);
...
<template>
  <section
    ...
    :class="{ 'is-outline-layout': currentModule === 'outline' }"
  ...
  >
    <div class="story-project-content-toolbar">
      ...
      <span v-if="currentModuleKicker" class="story-project-module-kicker">
        {{ currentModuleKicker }}
      </span>
      ...
      <div class="story-project-toolbar-actions" aria-label="项目操作">
        <button
          ...
          type="submit"
          :form="'story-basic-info-form'"
          title="保存基础信息"
        >
          保存
        </button>
        <span
          v-else-if="currentModule === 'outline'"
          class="story-project-toolbar-status"
          title="大纲编辑会自动保存"
        >
          大纲自动保存
        </span>
    ...
    <div
      class="story-project-module-main"
      :class="{
        'is-flush-top':
          currentModule === 'basic' ||
          currentModule === 'worldview' ||
          currentModule === 'roles' ||
          currentModule === 'outline',
      }"
    >
      ...
      <template v-else>
        ...
        <div class="story-asset-workspace">
          ...
          >
            ...
            <p
              v-if="importPending"
              class="story-asset-import-status"
              role="status"
            >
              故事导入任务已建立，等待文件上传与解析。
            </p>
