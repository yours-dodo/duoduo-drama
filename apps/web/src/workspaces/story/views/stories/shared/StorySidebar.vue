...
const navigation = [
  { href: '/', label: '创作空间' },
  { href: '/immersive', label: '沉浸式创作' },
  { href: '/templates', label: '模版库' },
];

const activePath = computed(() => {
  if (route.path === '/') return '/';
  if (route.path === '/immersive') return '/immersive';
...
function applySidebarState(collapsed: boolean) {
  ...
  document.documentElement.dataset.storySidebar = collapsed
    ? 'collapsed'
    : 'expanded';
}
...
<template>
  <aside
    class="story-floating-sidebar"
    :class="{ 'is-collapsed': sidebarCollapsed }"
    aria-label="故事工作区功能导航"
  >
    ...
    <div class="story-floating-sidebar-content" id="story-sidebar-navigation">
      <nav class="story-floating-nav">
        ...
        >
          ...
          <span class="story-floating-nav-short" aria-hidden="true">{{
      ...
      <div class="story-floating-recent-collapsed">
        ...
        >
          ...
          >
            <path
              d="M3.4 8.6A6.6 6.6 0 1 1 4.95 13"
              stroke="currentColor"
            ...
            />
            <path
              d="M3.4 4.2v4.4h4.4M10 6.8v3.6l2.4 1.4"
              stroke="currentColor"
              stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="1.5"
        ...
        <div
          ...
          class="story-floating-recent-popover"
          role="dialog"
          aria-label="最近对话"
        >
          ...
          <div class="story-floating-recent-popover-list">
            <span
              ...
              class="story-floating-recent-empty"
              >还没有聊过的对话</span
            >
            <RouterLink
              ...
              :to="
                toStoryRoutePath(
                  `/stories/${encodeURIComponent(conversation.projectId)}`,
                )
              "
      ...
      <section
        class="story-floating-recent"
        :class="{ 'is-collapsed': !recentExpanded }"
        aria-labelledby="story-recent-title"
      >
        <h2 id="story-recent-title" class="story-floating-group-title">
          <button
            class="story-floating-group-toggle"
            type="button"
            :aria-expanded="recentExpanded"
            @click="toggleRecent"
          >
            <span>最近</span>
            <span class="story-floating-group-chevron" aria-hidden="true"
              >⌄</span
            >
          </button>
        </h2>
        <div
          ...
          id="story-recent-list"
          class="story-floating-recent-list"
        >
          <span
            v-if="!recentConversations.length"
            class="story-floating-recent-empty"
    ...
    <div class="story-floating-user" aria-label="当前用户">
      <span class="story-floating-user-avatar" aria-hidden="true">{{
        userInitial
      }}</span>
