...
- `@duoduo/ai/protocols/duoduo-generation-v1` and `@duoduo/ai/providers/self-hosted-generation` expose the owned Generation Gateway catalog/task seam. Keep the public interface limited to model discovery plus create/get/cancel generation tasks: do not expose a `CloudGpuDriver`, cloud SDK, scheduler, or business `ArtifactStore`. Gateway GPU instance, container, node/host, and IP extensions are untrusted; discard them and emit a diagnostic rather than projecting them into public events or operation refs. Real infrastructure belongs behind the injected `DuoduoGenerationGateway` implementation.
