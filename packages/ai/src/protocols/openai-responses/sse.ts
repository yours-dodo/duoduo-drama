  ...
  try {
    for await (const chunk of body) {
      ...
      if (new TextEncoder().encode(buffer).byteLength > maxFrameBytes)
        throw invalidSse('SSE frame exceeds the configured limit');
      while (true) {
    ...
    if (error instanceof TypeError)
      throw invalidSse('SSE stream is not valid UTF-8');
    throw error;
  }
  if (buffer.trim().length > 0)
    throw invalidSse('SSE stream ended with an incomplete frame');
}
...
function invalidSse(message: string): AiRuntimeError {
  return new AiRuntimeError('OPENAI_INVALID_SSE', 'invalid_response', message);
}
